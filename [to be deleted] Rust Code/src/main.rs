...
pub fn main() {
    ...
    let mut output_time_series_string = String::new();
    for (index, argument) in env::args().enumerate().skip(1) {
        match index {
    ...
    if extract_all_measures_from_file(input_file, output_file) {
        ...
        println!("Could not extract all measures");
    }
}
...
impl UniqueDeveloperList {
    ...
    pub fn add_developer(&mut self, developer:String) -> bool {
    ...
    pub fn count_developers(&mut self) -> usize {
        self.developers.len()
    }
}
...
impl FeatureVector {
    ...
    pub fn new(measure: String) -> FeatureVector {
        FeatureVector {
            measure: measure,
            duration: 0,
            max_y: 0,
...
pub fn extract_all_measures_from_file(log_file_path: &Path, output_file: &Path) -> bool {
    ...
    for (line_num, commit) in csv_log_reader.records().enumerate() {
        let commit = commit.unwrap();
        // reject csv with too few/many columnszoon
        if commit.len() != EXPECTED_CSV_COLUMNS {
            panic!("Malformed line in {}", &line_num);
        }
        ...
        let current_author_date: i64 = match commit.get(AUTHOR_DATE) {
            None => {
                panic!("Malformed line in {}", &file_name);
            },
            Some(value) => {
                value.parse().unwrap()
            },
    ...
    for commit in csv_log_reader.records() {
        ...
        if integrator_activity_count[integration_week_number].add_developer(integrator_email) {
        ...
        if author_activity_count[committer_week_number].add_developer(author_email) {
        ...
        if character_count(&parent_hashes, ' ') > 0 {
    ...
    let integration_frequency_feature_vector = calculate_feature_vector_from_time_series(String::from("integrations"), &integration_frequency_timeseries);
    let integrator_activity_feature_vector = calculate_feature_vector_from_time_series(String::from("integrators"), &integrator_activity_timeseries);
    let commit_frequency_feature_vector = calculate_feature_vector_from_time_series(String::from("commits"), &commit_frequency_timeseries);
    let author_activity_feature_vector = calculate_feature_vector_from_time_series(String::from("commiters"), &author_activity_timeseries);
    let merge_frequency_feature_vector = calculate_feature_vector_from_time_series(String::from("merges"), &merge_frequency_timeseries);

    let mut wtr = match WriterBuilder::new().from_path(output_file) {
        Ok(x) => x,
        Err(x) => {panic!("Could not write to output file, perhaps it does not exist")}
    };

    let mut num_of_errors = 0; 
    for fv in vec![integration_frequency_feature_vector, integrator_activity_feature_vector, commit_frequency_feature_vector, author_activity_feature_vector, merge_frequency_feature_vector] {
        match wtr.serialize(fv) {
            Ok(x) => x,
            Err(x) => {num_of_errors += 1}
        }
    }
    if num_of_errors > 0 {
        println!("Program finished with {} errors", num_of_errors);
    }

    true
    //Ok(vec![integration_frequency_feature_vector, integrator_activity_feature_vector, commit_frequency_feature_vector, author_activity_feature_vector, merge_frequency_feature_vector])
}

fn calculate_feature_vector_from_time_series(measure: String, timeseries: &Vec<usize>) -> FeatureVector {
    let mut max_value = 0;
    let mut max_value_week = 0;
    let mut features = FeatureVector::new(measure); // TODO name should be "account_repo" (without ".log" at the end!)
...
fn detect_peaks_and_set_features(data_set: &Vec<usize>, mut features:FeatureVector) -> (Vec<i8>, FeatureVector) {
    ...
    sorted_data_set.sort_unstable();
    let (q25, q50, q75) = quartiles(&sorted_data_set);
